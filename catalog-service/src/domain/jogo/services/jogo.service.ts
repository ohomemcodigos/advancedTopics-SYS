...
export class JogoService {
  ...
  async findAll(): Promise<Jogo[]> {
    this.logger.log({
      msg: 'Buscando lista de todos os jogos',
      action: 'findAll',
    });
    ...
    if (cachedList) {
      ...
      this.logger.log({
        msg: 'Lista de jogos recuperada do cache (HIT)',
        action: 'findAll',
        cacheHit: true,
      }); // Regista HIT
      return cachedList;
    ...
    this.logger.log({
      msg: 'Lista de jogos recuperada da base de dados (MISS)',
      action: 'findAll',
      cacheHit: false,
    });
    return dadosBanco;
  ...
  async findOne(id: string): Promise<Jogo> {
    ...
    if (cachedJogo) {
      ...
      this.logger.log({
        msg: 'Jogo recuperado do cache (HIT)',
        action: 'findOne',
        cacheHit: true,
        jogoId: id,
      });
    ...
    this.cacheService.registrarMetricaTempo(false, tempoFim - tempoInicio); // Regista MISS
    this.logger.log({
      msg: 'Jogo recuperado da base de dados (MISS)',
      action: 'findOne',
      cacheHit: false,
      jogoId: id,
    });
  ...
  async update(id: string, dto: CreateJogoDto): Promise<Jogo> {
    this.logger.log({
      msg: 'Iniciando atualização de jogo',
      action: 'update',
      jogoId: id,
    });
    ...
    if (index === -1) {
      this.logger.warn({
        msg: 'Falha ao atualizar: Jogo não encontrado',
        action: 'update',
        jogoId: id,
      });
  ...
  async delete(id: string) {
    this.logger.log({
      msg: 'Iniciando exclusão de jogo',
      action: 'delete',
      jogoId: id,
    });
    const index = this.jogos.findIndex((j) => j.jogoId === id);
    ...
    if (index === -1) {
      this.logger.warn({
        msg: 'Falha ao excluir: Jogo não encontrado',
        action: 'delete',
        jogoId: id,
      });
      throw new NotFoundException('Jogo não encontrado');
