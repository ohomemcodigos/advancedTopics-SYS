...
describe('Pedido', () => {
  it('deve criar um pedido associado a um usuario externo com itens', () => {
    const item1 = new ItemPedido('SKU-123', 1, 199.9);
    const item2 = new ItemPedido('SKU-456', 2, 50.0);
  ...
  it('deve registrar o preco unitario no momento da compra', () => {
    const item = new ItemPedido('SKU-123', 1, 199.9);
  ...
  it('deve calcular o valor total corretamente', () => {
    const item1 = new ItemPedido('SKU-123', 2, 50.0);
    const item2 = new ItemPedido('SKU-456', 1, 100.0);

    const pedido = new Pedido('PED-002', 'USR-001', [item1, item2]);
  ...
  it('deve lancar erro ao criar pedido sem id', () => {
    expect(() => new Pedido('', 'USR-999', [])).toThrow(
      'ID do pedido é obrigatório.',
    );
  });

  it('deve cancelar um pedido pendente', () => {
    const pedido = new Pedido('PED-003', 'USR-001', []);
    pedido.cancelar();
